...
public class DlgOrdenSoporte extends JDialog implements ActionListener {
	...
	void listar() {
		...
		try {
			List<OrdenSoporte> lstOrdens = manager.createQuery(jpql, OrdenSoporte.class).getResultList();
			
			for (OrdenSoporte ordenSoporte : lstOrdens) {
				...
				imprimir("Detalle incidente........: " + ordenSoporte.getDetalleIncidencia());
				imprimir("-----------------------------\n");
